...
static volatile int8_t ticks_remaining = 0;
...
/* context switching */
static void exit_kernel(void) __attribute((naked,noinline));
static void enter_kernel(void) __attribute((naked,noinline));
...
static int kernel_create_task()
{
    ...
    /* The stack grows down in memory, so the stack pointer is going to end up
     * pointing to the location 32 + 1 + 1 + 2 + 2 = 37 bytes above the bottom, to make
     * room for (from bottom to top):
     *   the address of Task_Terminate() to destroy the task if it ever returns,
     *   the address of the start of the task to "return" to the first time it runs,
     *   register 31,
	...
	// stack_top[-33] = (uint8_t) _BV(SREG_I); /* set SREG_I bit in stored SREG. */
    /* stack_top[STACKCONTEXTSIZE-1] is r31. */

    /* We are placing the address (16/24-bit) of the functions
